...
func TestCodeLensCommand(t *testing.T) {
	...
	if lenses[0].Command != "editor.action.showReferences" {
		t.Errorf("expected showReferences command, got %q", lenses[0].Command)
	}
	if lenses[0].URI != "file:///project/doc.md" {
		t.Errorf("expected doc URI, got %q", lenses[0].URI)
	}
}

func TestCodeLensLocations(t *testing.T) {
	doc1 := document.New("file:///project/intro.md", 1, "# Introduction\n\nContent.\n")
	doc2 := document.New("file:///project/a.md", 1, "# A\n\n[link](intro.md)\n")
	...
	if len(lenses[0].Locations) != 1 {
		t.Fatalf("expected 1 location, got %d", len(lenses[0].Locations))
	}
	if lenses[0].Locations[0].URI != "file:///project/a.md" {
		t.Errorf("expected ref from a.md, got %q", lenses[0].Locations[0].URI)
	}
}
