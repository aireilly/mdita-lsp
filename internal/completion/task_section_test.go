...
func TestCompleteAttrOpen(t *testing.T) {
	tests := []struct {
		name      string
		text      string
		cursorPos document.Position
		...
		{
			name:      "bare { after bold offers domain elements",
			text:      "---\n---\n\n# Topic\n\nClick **OK**{",
			cursorPos: document.Position{Line: 5, Character: 16},
			wantItems: []string{".uicontrol", ".wintitle", ".menucascade", ".shortcut"},
		},
		{
			name:      "bare { after code offers code domain elements",
			text:      "---\n---\n\n# Topic\n\nRun `cmd`{",
			cursorPos: document.Position{Line: 5, Character: 12},
			wantItems: []string{".filepath", ".cmdname", ".codeph"},
		},
		{
			name:      "bare { after italic offers italic domain elements",
			text:      "---\n---\n\n# Topic\n\nSee *book*{",
			cursorPos: document.Position{Line: 5, Character: 13},
			wantItems: []string{".cite"},
		},
		{
			name:      "bare { in heading offers heading classes",
			text:      "---\n---\n\n# My Topic {",
			cursorPos: document.Position{Line: 3, Character: 14},
			wantItems: []string{".task", ".concept", ".reference"},
		},
		{
			name:      "bare { in heading offers conditional attrs",
			text:      "---\n---\n\n# My Topic {",
			cursorPos: document.Position{Line: 3, Character: 14},
			wantItems: []string{"audience", "platform", "product"},
		},
		{
			name:      "bare { in heading with input filters to matching attrs",
			text:      "---\n---\n\n# My Topic {au",
			cursorPos: document.Position{Line: 3, Character: 16},
			wantItems: []string{"audience"},
		},
	}
